...
const softDeleteProduct = async (req, res) => {
  try {
    ...
    let result = await dbService.update(Product, query,updateBody);
    if (!result){
      return res.recordNotFound();
    }
...
const deleteProduct = async (req, res) => {
  const result = await dbService.deleteByPk(Product, req.params.id);
  return  res.success({ data :result });
};
...
const deleteManyProduct = async (req, res) => {
  try {
    ...
    let deletedProduct = await dbService.destroy(Product,query);
    return res.success({ data :{ count :deletedProduct.length } });
  }
  catch (error){
    return res.internalServerError({ message:error.message });  
  }
...
const softDeleteManyProduct = async (req, res) => {
  try {
    ...
    const options = {};
    let updatedProduct = await dbService.update(Product,query,updateBody, options);
    if (!updatedProduct) {
      return res.recordNotFound();
    }
    return  res.success({ data :{ count: updatedProduct.length } });
